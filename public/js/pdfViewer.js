...
(async function () {
    ...
    async function renderPage(num) {
        ...
        const pageWrapper = flipbookEl.querySelector(
            '.page[data-page="' + num + '"]'
        );
        if (pageWrapper) {
            pageWrapper.innerHTML = "";
    ...
    function initTurn() {
        ...
        if ($("#flipbook").data("turn")) {
            try {
                $("#flipbook").turn("destroy").removeData("turn");
            } catch (e) {}
        }
        ...
        if (scaleX < 1) {
            ...
            $("#flipbook")
                .parent()
                .css({ height: Math.round(pageHeight * scaleX) + "px" });
        } else {
    ...
    if (typeof $.fn.turn === "function") {
        ...
        statusEl.textContent =
            "turn.js active — page " + current + " / " + pdf.numPages;
        ...
        async function showSpread(page) {
            ...
            const leftCanvas = flipbookEl.querySelector(
                '.page[data-page="' + leftNum + '"] canvas'
            );
            const rightCanvas = flipbookEl.querySelector(
                '.page[data-page="' + rightNum + '"] canvas'
            );
            ...
            statusEl.textContent =
                ...
                leftNum +
                " - " +
                (rightNum <= pdf.numPages ? rightNum : "-") +
                " / " +
                pdf.numPages;
        }

        async function flipNext() {
            if (current + 2 > pdf.numPages + (pdf.numPages % 2 === 0 ? 0 : 1))
                return;
            ...
            const srcCanvas =
                flipbookEl.querySelector(
                    '.page[data-page="' + rightNum + '"] canvas'
                ) || renderedCanvases[rightNum];
            if (srcCanvas) {
                ...
                requestAnimationFrame(function () {
                    overlay.classList.add("animate", "right");
                });
                ...
                statusEl.textContent =
                    ...
                    current +
                    " - " +
                    (current + 1 <= pdf.numPages ? current + 1 : "-") +
                    " / " +
                    pdf.numPages;
        ...
        async function flipPrev() {
            ...
            if (srcCanvas) {
                ...
                sheet.appendChild(overlay);
                requestAnimationFrame(function () {
                    overlay.classList.add("animate", "left");
                });
